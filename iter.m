parameters=mainTMD('m',0.45,'psi',-0.3329/(2*pi)*360,'V',4.428,'w',20,'theta',4,'nu',[1,1]);
% tshell=3;
% Ushell=0;
...
epsilon=10;
clear en;
[energyall,wfall]=energyMF_init(kx3list,ky3list,[neighborlist{1:tshell+1}],0*[t{1:tshell+1}],[neighborlist{1:Ushell+1}],real([U{1:Ushell+1}])/epsilon,parameters);
for i=1:50
spin=spintexture(energyall,wfall,parameters)
en(i)=totalenergy(kx3list,ky3list,[neighborlist{1:tshell+1}],0*[t{1:tshell+1}],[neighborlist{1:Ushell+1}],real([U{1:Ushell+1}])/epsilon,energyall,wfall,parameters);
[energyall,wfall]=energyMF(kx3list,ky3list,[neighborlist{1:tshell+1}],0*[t{1:tshell+1}],[neighborlist{1:Ushell+1}],real([U{1:Ushell+1}])/epsilon,energyall,wfall,parameters);
energyall_o=energyall;
wfall_o=wfall;
end
% i=0;
% gap=0;
