function sweep_vz_T(nu,vzlist,Tlist,theta,hole,perturb,perturbnear,filename,epsilon,Ush)
    ...
    parfor Ti=1:NT
        for vzi=1:Nvz
            parameters=mainTMD_2('m',0.45,'psi',-0.3329/(2*pi)*360,'V',4.428,'w',20,'theta',theta,'T',Tlist(thetai),'d',inf,'vz',vzlist(vzi),'nu',nu,'hole',hole,'perturb',perturb,'perturbnear',perturbnear);
            kxlist=zeros(1,n^2);
            kylist=zeros(1,n^2);
            counter=1;
            for xindex=1:n
    ...
    save(sprintf('phase%d,%d_h%d_vz_ep%d_theta%.2f_U%d.mat',nu(1),nu(2),hole,epsilon,thetalist(thetai),Ush),'nu','final','spin','vzlist','gap','innergap','Tlist','finali','ch');
