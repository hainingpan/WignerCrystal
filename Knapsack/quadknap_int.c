...
typedef double     stype;   /* sum of profit or weight  */
...
double povercomp(ntype *a, ntype *b) {return DET(pover[*b],w[*b],pover[*a],w[*a]);}
double icomp(ntype *a, ntype *b) { return *a - *b; }
double ecomp(lpitem *a, lpitem *b) { 
  if (a->e == b->e) return 0; 
  return (a->e < b->e ? 1 : -1); 
}
...
static void invertorder(double *rev, double *org)
{
  int i;
...
int quadknap(int no, int cap, double *ptab, double *wtab, double *xtab)
{
  int i, j;
  long time, ltime;
  stype ps, ws;
  ...
  memcpy(p, ptab, sizeof(double) * MSIZE * MSIZE);
  memcpy(w, wtab, sizeof(double) * MSIZE);
  ...
  memcpy(xtab, xstar, sizeof(double) * MSIZE);
  return z;
}
